...
object IdConverter {
  ...
  def CreateAlphabet(excludedChars: List[String])(base: Int, prefix: String, stringRanges: List[Range]): LinkAlphabet = {
    ...
    def findNextAlphabetChar(currentBase: Int, currentAlphabet: LinkAlphabet, currentSet: Set[Int]): LinkAlphabet = {
      ...
      def randomChar: Int = {
        ...
        if (currentAlphabet.alphabet.contains(nro))
          randomChar
        else
          nro
      }
      if (currentBase == base)
        ...
        val nro = randomChar
        findNextAlphabetChar(currentBase + 1,
          LinkAlphabet(currentAlphabet.prefixAlphabet,
            currentBase + 1,
            currentAlphabet.alphabet + ((nro, currentBase)),
            currentAlphabet.stringRanges),
            currentSet.-(nro))
      }
    }
  ...
  def GetShortLink(alpha: LinkAlphabet)(id: String, idBase: Int): String = {
    ...
    val toProcess = if (brokenId.isEmpty) List(id) else brokenId
    @tailrec
    def generateLink(currentList: List[String], currentLink: String): String = {
      currentList match {
        case h :: t =>
          if (t.nonEmpty)
            generateLink(t, currentLink + encode(h) + rangeSeparator)
          else
            generateLink(t, currentLink + encode(h))
  ...
  def GetObjectId(alpha: LinkAlphabet)(converter: Int => String)(sid: String): Box[String] = {
    ...
    def completeZeros(symbol: String, index: Int): String = {
      (1 to (if (alpha.stringRanges.nonEmpty) alpha.stringRanges(index).length - symbol.length - 1 else 0)).foldLeft("")((p, zero) => p + "0") + symbol
    }
